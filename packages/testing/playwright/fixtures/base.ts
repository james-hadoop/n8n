...
type WorkerFixtures = {
	...
	capability: Capability | Capability[] | ContainerConfig | null;
};
...
>({
	...
	capability: [
		null as Capability | Capability[] | ContainerConfig | null,
	...
	containerConfig: [
		async ({ capability }, use, workerInfo) => {
			...
			// Build capability config from string(s) or use custom config directly
			let capabilityConfig: ContainerConfig = {};
			if (capability) {
				if (typeof capability === 'string') {
					// Single capability string
					capabilityConfig = CAPABILITIES[capability] ?? {};
				} else if (Array.isArray(capability)) {
					// Array of capability strings
					capabilityConfig = Object.assign({}, ...capability.map((c) => CAPABILITIES[c] ?? {}));
				} else {
					// Custom ContainerConfig object
...
Worker: capability → containerConfig → n8nContainer → [backendUrl, frontendUrl, dbSetup, chaos]
