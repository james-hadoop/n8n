...
### 1. Use capability strings (enables worker reuse)

```typescript
// CORRECT - string capability enables worker reuse
test.use({ capability: 'proxy' });

// Multiple capabilities
test.use({ capability: ['proxy', 'email'] });

// Custom config (use object form)
test.use({
  capability: {
